...
import { FileTypeEnum, IAttachment } from 'Entities/attachment'
import { useAttachments, useTopikLevelAttachment } from 'Hooks'
import { updateDialogEvent } from 'Models'
import { useCallback, useEffect, useMemo } from 'react'
import { useForm } from 'react-hook-form'
import { ControllerHookProps, IAutoCompleteOption, LangError } from 'Types/app'
import * as yup from 'yup'

type Values = Pick<IAttachment, 'fileNameUz' | 'fileNameRu' | 'fileNameKr'> & {
  file: null | File
  url: string | null
  type: IAutoCompleteOption | null
}
type Props = ControllerHookProps

export function useTopikLevelFileForm({ initList, extraId, detailId }: Props) {
  const {
    create,
    listQuery: { refetch },
  } = useTopikLevelAttachment({ initList, extraId })
  ...
  const schema = yup.object().shape({
    ...
    type: yup.mixed().required(APP.REQUIRED_FIELD),
  })
  ...
  const form = useForm<Values>({
    defaultValues: {
      ...
      fileNameUz: '',
      url: '',
      type: { label: FileTypeEnum.ZIP, value: FileTypeEnum.ZIP },
    },
      ...
      if(values.type){
        if (detailId) {
          const data: any = {
            ...
            fileTypeEnum: values.type.value
          }
          update.mutate({
            data,
          })
          ...
          if (values.file && extraId) {
            ...
            formData.append('levelOfTopicId', extraId)
            ...
            formData.append('fileTypeEnum', String(values.type.value))
  ...
  useEffect(() => {
    if (detailId && data) {
      ...
      setValue('type', { label: String(data.fileTypeEnum), value: String(data.fileTypeEnum) })
    }
