...
import { useAttachments, useStudyMaterialAttachment } from 'Hooks'
...
import { StudyMaterialFileForm } from './StudyMaterialFileForm'
...
export const StudyMaterialFileList = ({ id }: Props) => {
  const [removeId, setRemoveId] = useState<number | null>(null)
  const {
    listQuery: { data, refetch, isFetching, isLoading },
  } = useStudyMaterialAttachment({ initList: true, extraId: id })
  ...
  const onEdit = useCallback(
    (detailId: number) => {
      updateDialogEvent({
        ...
        open: true,
        content: <StudyMaterialFileForm id={id} detailId={String(detailId)} />,
      })
    },
    [id],
  )
