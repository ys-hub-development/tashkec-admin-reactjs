...
import { useCultureConfig } from 'Views/Culture/Hooks/useCultureConfig'
import { ICulture } from 'Entities/culture'
import { useCulture } from 'Hooks'

type Values = Omit<ICulture, 'id'>
type Props = ControllerHookProps

export function useCultureForm({ initList, detailId }: Props) {
  ...
  const action = useCallback(
    (field?: ApiActionParamType) => {
      if (!field?.['err']) {
        navigate(`/${CulturePath.main}/${subPath}`)
      }
    },
    [ navigate, subPath ],
  )
