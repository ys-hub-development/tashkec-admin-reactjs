...
import { useAnswerForm } from 'Views/Faq/Hooks'

export const AnswerForm = () => {
  const { answerId } = useParams<IParams>()
  const navigate = useNavigate()
  const { form, onSubmit, langError, disabled, isLoading } = useAnswerForm({ initList: false, detailId: answerId })
          ...
          <Grid container rowSpacing={3}>
            ...
            <Grid item xs={12} marginTop={1}>
              <Stack direction='row' spacing={2}>
                ...
                <Button className='light' size='large' onClick={() => navigate(`/${FaqPath.main}/${FaqPath.answers}`)}>
                  {APP.CANCEL}
                </Button>
              </Stack>
            </Grid>
